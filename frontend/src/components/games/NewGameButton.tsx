...
export default function NewGameButton() {
  ...
  return (
    <div className="flex flex-row flex-center rounded-lg bg-gradient-to-tr from-blue-light to-blue-dark ">
      <button
        onClick={handleClick}
        className={`h-12 flex text-white font-medium transition-[width] duration-300 shadow-xl items-center
          ${isAnimating ? "w-60 overflow-hidden delay-75" : "w-12 flex-center"}
          ${isOpen ? "w-36 transition-none" : ""}
`}
      >
        <svg
          width="29"
          ...
          className={
            isAnimating
              ? "animate-spin animate-once animate-duration-300 m-3"
              : ""
          }
        >
          <path
        ...
        {isOpen && (
          <span className="text-xl animate-fade-left animate-duration-150">
            Vytvořit hru
          </span>
        )}
      </button>
      {isOpen && (
        <button
          onClick={handleClick}
          className="h-12 w-24 flex flex-center text-white font-medium shadow-xl items-center border-l-2 border-white"
        >
          <span className="text-xl animate-fade-right animate-duration-150">
            Vytvořit hru
          </span>
        </button>
      )}
